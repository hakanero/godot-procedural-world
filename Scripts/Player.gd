...
onready var sky_b=get_tree().get_root().get_node("World/Sky")

# Called when the node enters the scene tree
func _ready() -> void:
	var iChannel=sky_b.get_viewport().get_texture()
	cam.environment=load("res://Materials/Environemnt.tres") as Environment
	cam.environment.background_sky.set_panorama(iChannel)
